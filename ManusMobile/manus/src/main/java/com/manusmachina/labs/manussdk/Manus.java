...
public class Manus extends Service {
    ...
    private volatile Glove mGloveLeft = null;
    private volatile Glove mGloveRight = null;
    ...
    private void connect(BluetoothDevice dev) {
        ...
        if (mGloveLeft != null) {
            if (mGloveLeft.mGatt.getDevice().getAddress().equals(dev.getAddress())) {
                mGloveLeft.mGatt.connect();
                return;
            }
        }
        if (mGloveRight != null) {
            if (mGloveRight.mGatt.getDevice().getAddress().equals(dev.getAddress())) {
                mGloveRight.mGatt.connect();
                return;
            }
        }
    ...
    private GloveCallback mGloveCallback = new GloveCallback() {
        ...
        public void OnGloveConnected(Glove glove, boolean serviceFound) {
            if (serviceFound) {
                if (glove.getHandedness() == Glove.HAND.LEFT && mGloveLeft == null) {
                    mGloveLeft = glove;
                } else if (glove.getHandedness() == Glove.HAND.RIGHT && mGloveRight == null) {
                    mGloveRight = glove;
                } else {
                    glove.close();
                }
        ...
        public void OnChanged(Glove glove) {
            for (OnGloveChangedListener listener : mOnGloveChangedListeners) {
                listener.OnGloveChanged(glove.getHandedness(), glove);
            }
        }
    };

    public interface OnGloveChangedListener {
        void OnGloveChanged(Glove.HAND hand, Glove glove);
    }
    ...
    public class GloveBinder extends Binder {
        public Glove getGlove(Glove.HAND hand) {
            if (hand == Glove.HAND.LEFT && mGloveLeft != null)
                return mGloveLeft;
            if (hand == Glove.HAND.RIGHT && mGloveRight != null)
                return mGloveRight;
